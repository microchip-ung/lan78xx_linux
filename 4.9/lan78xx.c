...
#define DRIVER_NAME	"lan78xx.napi20201007"
#define DRIVER_VERSION	"1.0.4_napi20201007"
...
#define LAN78XX_EEPROM_EXTENT_VALID(e)	(((e)->len <= MAX_EEPROM_SIZE) && \
					 ((e)->offset < MAX_EEPROM_SIZE) && \
					 (((e)->len + (e)->offset) <= MAX_EEPROM_SIZE))
				      ...
				      struct ethtool_eeprom *ee, u8 *data)
{
	...
	if ((ee->magic == LAN78XX_EEPROM_MAGIC) &&
	    LAN78XX_EEPROM_EXTENT_VALID(ee)) {
		if ((ee->offset == 0) &&
...
static void lan78xx_get_regs(struct net_device *netdev,
...
static void lan78xx_init_mac_address(struct lan78xx_net *dev)
{
	...
	/* Add MAC address to entry 0 of perfect address filter table */
...
static int lan78xx_change_mtu(struct net_device *netdev, int new_mtu)
{
	...
	if ((new_mtu < LAN78XX_MIN_MTU ) || (new_mtu > LAN78XX_MAX_MTU))
		return -EINVAL;
...
static int lan78xx_resume(struct usb_interface *intf)
{
	...
	if (dev_open) {
		...
		if (dev->urb_intr) {
			if ((ret = usb_submit_urb(dev->urb_intr, GFP_KERNEL)) < 0) {
			if (ret == -ENODEV)
				netif_device_detach(dev->net);
